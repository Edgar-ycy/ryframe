...
Windows 本机直接运行 RustFS 时，回收站清理任务可能周期性输出
`cleanup_deleted_objects ... PermissionDenied("拒绝访问")` 或“主 checkpoint 文件不存在/损坏”。
这类错误来自 RustFS 数据目录（本机示例为 `D:\rustfs`）的权限或初始化状态，不是 RyFrame 业务代码问题：

- 确认 RustFS 进程的运行账号对数据目录拥有完整修改权限，且没有杀毒软件或文件句柄占用。
- 清理任务失败会把删除操作延后到下一个周期；对象仍可正常读写，但应尽快恢复目录权限，
  避免回收站持续膨胀。
- checkpoint 损坏时先备份 `D:\rustfs`，停止 RustFS 后让启动流程重建 checkpoint；不要直接
  删除整个数据目录。
- 生产环境优先使用容器化 RustFS，并将 `/data` 卷交给容器内声明的非 root 用户，避免宿主
  目录权限漂移。

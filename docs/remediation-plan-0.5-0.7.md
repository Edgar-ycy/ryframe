...
| i18n、OTel、URL 版本与读副本 | 部分完成 | 双语资源、语言协商、`Content-Language`、`Vary: Accept-Language`、OTel、`/api/v1` 与读一致性选择已实现；未知版本和无版本 API 路径已统一为 JSON `404`。当前通用 HTTP 成功与错误消息仍可能固定输出中文，英文响应语义尚未闭环。API 与独立 Worker 已共用日志初始化，文件模式按日滚动并执行有界保留，生产容器默认 stdout。两类进程的 `/readyz` 均只读后台探测生成的有时效内存快照，过期时 fail-closed，请求路径不再访问依赖；Worker 对象存储明确为 `not_required`。隔离环境已验证健康副本参与轮询读取，副本不可达时 API 仍就绪并切换为 `primary_fallback`；连续三次失败摘除、两次成功恢复、Strong 固定主库和 schema 落后排除仍缺可重复运行证据。OTel 已通过不响应的本机回环端点完成确定性故障验收，仍需用真实 Collector 验证完整父子链路及停止、恢复过程。 |
| 通用消息中心 | 待核验 | 消息表、票据、服务端持久化收件箱、ack/read 与前端连接均已具备；容量与生命周期由强类型 `MessagingConfig` 注入，收件人通过同事务 `INSERT … SELECT` 固化。当前只能确认主体实现存在，尚无可重复执行的双实例运行验收证据；仍需验证断线补拉、去重、ticket 过期与重放、错误 Origin、Redis 故障、1013 慢消费者、90 天清理和双实例租户隔离。在这些证据完成前，不将双实例消息验收标记为已完成。 |
| 源码发布与生产 Compose | 部分完成 | GitHub Release 已明确只保留平台自动生成的源码 ZIP/TAR，不分发任何平台镜像或自定义附件；OCI 多架构镜像、SBOM、Cosign 签名和 GHCR 交付已由纯源码范围裁决覆盖。生产 Compose 已将 API、迁移和独立 Worker 与外部托管依赖分离；仍需由部署环境完成稳定标签源码构建、扫描、摘要固化和升级演练。 |
| 0.7 发布验收 | 部分完成 | OTel 不可达时的本地确定性故障验收，以及读副本健康选择与不可达主库回退的基础验证已经完成；完整 Collector 链路、读副本摘除与恢复、消息中心双实例及其断线、重放、Redis 故障、慢消费者和租户隔离场景仍待可重复运行验收。另须完成生产源码构建、升级和回滚演练。 |
...
1. `0.6.0` 的双仓契约、稳定标签、精确提交 CI 和纯源码联合发布已经收口，部署环境正式切换保留为外部验收。
2. `0.7.0` 的五项安全依赖升级已经分波完成，`base64` 与 `password-hash` 按已登记风险延期。
3. 收口 `0.7.0` 的架构、i18n、OTel 与读副本运行验收。
4. 补齐消息中心双实例、断线重连、故障恢复、慢消费者和租户隔离的可重复运行证据。
5. 验证 `0.7.0` 纯源码 Release，并在授权生产环境完成源码构建、扫描、摘要固化、迁移切换和回滚演练。

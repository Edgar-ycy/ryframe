...
一致性，仅用于明确的本地开发或隔离验证，不能用于多实例生产部署。
...
Refresh Family 是设备会话是否有效的唯一可靠事实来源。每个 Family 由稳定 `sid` 标识，并绑定
租户、用户、当前/前一 refresh JTI、撤销状态和不超过七天的绝对过期时间。认证中间件在每个
受保护请求上同时校验 `sid + tenant_id + user_id`；会话一经撤销，后续 access token 请求不能
仅凭未过期 JWT 继续访问。
...
浏览器、操作系统、IP、位置和最近活动时间保存在在线用户 metadata 中，只用于设备列表展示。
第三阶段升级后，新登录同时写入新租户/用户索引；为了不强制升级前用户退出，设备列表在兼容期
同时读取新索引与旧在线用户 metadata，再用 Refresh Family 逐项验证。双读必须至少覆盖当前
最大 Refresh TTL，即发布后七天；在该窗口结束前不得删除旧 metadata 扫描或旧索引清理逻辑。
如果历史异常数据使候选超过 256，批量撤销拒绝扩大脚本输入，调用方应改用经过相同身份校验的
单设备撤销接口逐一收敛。
...
平台租户容量治理也保持 API、Service、Repository 分层。API 负责系统租户与权限边界、分页参数和
公开 DTO；Service 组合主库资源用量、Redis 当前窗口与容量状态；Repository 先分页租户，再以当前页
租户 ID 执行一次条件聚合，不允许逐租户计数。用户、角色、文件元数据和辅助状态均从主库强一致
读取；Redis 只提供当前请求窗口的只读快照，故障时局部返回 `unknown`，不能使主库用量查询失败。
整体容量状态只取用户、角色、存储三项，零配额统一表示无限制。平台权限分为 `tenant:list` 与仅系统
...
```mermaid
flowchart LR
    A["Agent 请求"] --> B["可信代理 IP、请求 ID、预认证 IP 防刷"]
    B --> C["API Key；可选委托令牌"]
    C --> D["Redis 七维频率与并发决策"]
    D --> E["租户、服务账号、凭据共享锁"]
    E --> F["固定能力与双主体权限交集"]
    F --> G["双主体数据范围交集"]
    G --> H["注册表只读 Repository"]
    H --> I["最终响应大小校验"]
    I --> J["同事务服务访问审计"]
    J --> K["主库提交后原样响应"]
```

直接模式只有服务账号主体；委托模式同时存在服务账号和被代表用户。能力决策必须满足服务账号有效
权限、用户有效权限、委托记录中的编译期能力白名单以及能力自身 direct/delegated 声明。行级范围
不能用任一主体替代另一主体：用户和部门查询计算两个范围的集合交集，岗位和字典当前只在双方范围
都为全部时开放。能力发现仍基于同一决策函数，只列出真正可用的固定能力，不允许客户端提交路径、
SQL、过滤器、排序、operationId 或权限码形成新业务逻辑。

安全决策与查询固定使用主库强一致事务。锁序为租户共享锁、服务账号共享锁、凭据共享锁、可选委托
共享锁、能力白名单共享锁，再读取授权快照并执行查询。成功审计与查询共享事务，事务提交前不向
客户端返回成功字节；失败路径另行写入最小审计，审计不可用时 fail-closed。服务账号状态、角色、
...
保存带用途域分离的 HMAC-SHA-256 结果和 Pepper 版本，Keyring 与 JWT Secret 必须独立。
...
68. 平台租户治理保留旧基础列表并增加有界分页、详情和独立用量接口；三类资源用量从主库批量聚合，当前请求窗口从 Redis 只读快照局部补充。容量筛选与用量字段由系统租户专用权限保护，Redis 故障只产生请求窗口 `unknown`；查询不建立请求历史、不轮询完整租户列表，也不向 Prometheus 增加租户高基数标签。

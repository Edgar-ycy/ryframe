...
#[tokio::test]
async fn filtered_menu_query_is_paginated_in_the_repository() {
    let db = common::setup_test_db().await;
    let repo = MenuRepository;

    for (id, name, status, sort) in [
        (20, "系统用户", "1", 1),
        (21, "系统角色", "1", 2),
        (22, "业务报表", "0", 3),
    ] {
    ...
    assert_eq!(page.records[0].name, "系统角色");
}
...
async fn find_tree_by_permission_codes_includes_matching_menus_and_ancestors() {
    ...
    let tree = repo
        .find_tree_by_permission_codes(&db, "system", &[String::from("system:user:list")])
        .await
        .unwrap();
...
async fn find_by_permission_codes_handles_empty_and_wildcard_permissions() {
    ...
    let empty = repo
        .find_by_permission_codes(&db, "system", &[])
        .await
        .unwrap();
    assert!(empty.is_empty());

    let wildcard = repo
        .find_by_permission_codes(&db, "system", &[String::from("*:*:*")])
        .await
        .unwrap();
...
async fn button_permission_does_not_grant_parent_page_access() {
    ...
    let button_only = repo
        .find_tree_by_permission_codes(&db, "system", &[String::from("system:menu:add")])
        .await
        .unwrap();

...
pub fn owner_id_condition<C>(owner_column: C, ctx: &DataScopeContext) -> Option<Condition>
where
    C: ColumnTrait,
{
    match ctx.scope {
        ...
        DataScope::Dept => Some(match ctx.dept_id {
            Some(dept_id) => owner_in_departments(owner_column, vec![dept_id], false, ctx),
        ...
        DataScope::DeptAndChildren => ctx
            ...
            .map(|dept_id| {
                let dept_ids = Query::select()
                    ...
                    .and_where(
                        crate::entities::dept::Column::TenantId
                            .eq(ryframe_core::current_tenant_id()),
                    )
                    .and_where(
                    ...
                    .take();
                let user_ids = base_user_id_query()
...
pub fn owner_username_condition<C>(owner_column: C, ctx: &DataScopeContext) -> Option<Condition>
where
    C: ColumnTrait,
{
    ...
    let mut users = base_user_name_query();
    match ctx.scope {
...
fn base_user_id_query() -> sea_orm::sea_query::SelectStatement {
    Query::select()
        .column(user::Column::Id)
        .from(user::Entity)
        .and_where(user::Column::TenantId.eq(ryframe_core::current_tenant_id()))
        .and_where(user::Column::DelFlag.eq(user::Model::DEL_FLAG_NORMAL))
        .to_owned()
}

fn base_user_name_query() -> sea_orm::sea_query::SelectStatement {
    Query::select()

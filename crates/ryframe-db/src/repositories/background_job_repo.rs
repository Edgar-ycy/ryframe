...
mod tests {
    ...
    fn claim_query_uses_skip_locked_on_mysql() {
        ...
        let statement = BackgroundJobRepository::claimable_query(now)
            .lock_with_behavior(LockType::Update, LockBehavior::SkipLocked)
            .build(DbBackend::MySql);

        assert!(statement.sql.contains("FOR UPDATE SKIP LOCKED"));
        assert!(statement.sql.contains("`available_at` <= ?"));
        assert!(statement.sql.contains("`attempts` < `max_attempts`"));
    }

    #[test]
    fn expired_lease_dead_query_records_a_safe_reason() {
        let now = Utc
            .with_ymd_and_hms(2026, 7, 26, 8, 0, 0)
            .single()
            .expect("valid timestamp");
        let statement =
            BackgroundJobRepository::expired_lease_dead_query(now).build(DbBackend::MySql);

        assert_eq!(EXPIRED_LEASE_DEAD_ERROR, "任务租约已过期，处理结果未知");
        assert!(statement.sql.contains("`last_error` = ?"));
        assert!(statement.sql.contains("`lease_until` <= ?"));
        assert!(statement.sql.contains("`attempts` >= `max_attempts`"));
    }

    #[test]
    fn enqueue_validation_rejects_invalid_budget_and_key() {
        let mut job = EnqueueBackgroundJob {
            tenant_id: None,
            job_type: "audit.record".into(),
    ...
    fn error_truncation_preserves_utf8_boundaries() {
        let source = "错".repeat(4_000);
        let truncated = truncate_error(&source);
        assert!(truncated.len() <= 8 * 1024 + "…".len());
        assert!(truncated.ends_with('…'));
        assert!(truncated.is_char_boundary(truncated.len()));
    }

    #[test]
    fn duplicate_errors_are_detected_without_matching_other_database_errors() {
        let duplicate = sea_orm::DbErr::Custom("ERROR 1062 duplicate entry".into());
        let unrelated = sea_orm::DbErr::Custom("connection reset".into());
        assert!(is_duplicate_key_error(&duplicate));
        assert!(!is_duplicate_key_error(&unrelated));
    }

    #[test]
    fn manual_retry_resets_attempt_budget_and_guards_dead_tenant_row() {
        let now = Utc
            .with_ymd_and_hms(2026, 7, 26, 8, 0, 0)
            .single()
            .expect("valid timestamp");
        let statement =
            BackgroundJobRepository::retry_dead_query("tenant-a", 42, now).build(DbBackend::MySql);

...
fn validate_deletion_candidates(
...
mod tests {
    ...
    fn active_lease_rejects_whole_batch_but_expired_lease_does_not() {
        ...
        validate_deletion_candidates(&exports, &[expired], now)
            .expect("过期租约不能永久阻止终态清理");
    }

    #[test]
    fn foreign_missing_or_hidden_member_rejects_the_whole_batch_as_not_found() {
        assert!(matches!(

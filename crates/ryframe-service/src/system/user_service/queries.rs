...
mod export_tests {
    ...
    use super::{calculate_export_authorization_fingerprint, validate_export_cursor_batch};

    fn actor(data_scope: DataScope) -> ActorContext {
        ActorContext {
            user_id: 7,
    ...
    #[test]
    fn one_hundred_thousand_cursor_rows_are_bounded_and_strictly_ordered() {
        let mut cursor = None;
        let mut accumulated = 0_usize;
        for start in (1_i64..=100_000).step_by(1_000) {
            let batch = (start..start + 1_000).collect::<Vec<_>>();
            cursor = validate_export_cursor_batch(&batch, cursor, accumulated, 100_000, |id| *id)
                .unwrap();
            accumulated += batch.len();
        }
        assert_eq!(cursor, Some(100_000));
        assert!(
            validate_export_cursor_batch(&[100_001], cursor, accumulated, 100_000, |id| *id)
                .is_err()
        );
    }

    #[test]
    fn narrowed_data_scope_changes_the_authorization_fingerprint() {
        let permissions = vec!["system:user:export".to_owned()];
        let wide = calculate_export_authorization_fingerprint(
            4,
            9,
            &actor(DataScope::All),
            &[],
        ...
        .unwrap();
        let narrow = calculate_export_authorization_fingerprint(
            4,
            9,
            &actor(DataScope::SelfOnly),
            &[],
            &permissions,
        )
        .unwrap();

...
#[cfg(test)]
mod tests {
    use super::{AuditRequestContext, OperLogStatus, RecordOperLogCommand};

    fn command() -> RecordOperLogCommand {
        RecordOperLogCommand {
            title: "用户管理".into(),
            business_type: "UPDATE".into(),
            method: "PUT /system/users/{id}".into(),
            request_method: "PUT".into(),
            oper_name: "admin".into(),
            oper_url: "/system/users/{id}".into(),
    ...
    fn final_event_preserves_request_and_event_correlation_ids() {
        ...
        let event = context.event(OperLogStatus::Failure, Some("HTTP 500".into()));
        assert_eq!(event.tenant_id, "system");
        assert_eq!(event.command.error_msg.as_deref(), Some("HTTP 500"));
        assert!(matches!(event.command.status, OperLogStatus::Failure));
    }

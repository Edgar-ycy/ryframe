...
const KEY_PREFIX: &str = "ryframe:agent-limit:";

#[derive(Clone)]
struct RedisAgentLimiter {
    redis: RedisClient,
}

struct RedisAgentConcurrencyLease {
    redis: RedisClient,
    key: String,
    owner: String,
}

pub fn redis_limiter(redis: RedisClient) -> Arc<dyn AgentLimiter> {
...
impl RedisAgentLimiter {
    async fn guard_ip(&self, ip: &str, limit: u32) -> AppResult<()> {
    ...
    ) -> AppResult<RedisAgentConcurrencyLease> {
        let mut dimensions = Vec::<(String, u32, u32)>::new();
        // 预认证 IP 桶已经承担独立 IP 维度；完整决策只读取该桶，避免重复计数。
        dimensions.push((

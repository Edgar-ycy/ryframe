...
mod tests {
    ...
    fn row(status: &str, expires_at: Option<DateTime<Utc>>) -> sys_file::Model {
        let now = Utc::now();
        sys_file::Model {
            id: 1,
            tenant_id: "system".into(),
            original_name: "legacy.txt".into(),
            storage_name: "legacy.txt".into(),
            storage_path: "system/legacy.txt".into(),
            bucket: "uploads".into(),
            file_url: "uploads/system/legacy.txt".into(),
            file_size: 1,
            content_type: "text/plain".into(),
            file_sha256: "a".repeat(64),
            upload_by: None,
            upload_status: status.into(),
            reservation_token: Some("token".into()),
            reservation_expires_at: expires_at,
            del_flag: LEGACY_RESERVED_FLAG.into(),
            created_at: now,
            updated_at: now,
        }
    }
    ...
    #[test]
    fn apply_requires_explicit_confirmation_and_database() {
        let error = parse_args([
            "backfill-sha256".into(),
            "apply".into(),
            "--database".into(),
            "ryframe".into(),
        ])
        .unwrap_err();
        assert!(error.to_string().contains(APPLY_CONFIRMATION));
        ...
        let arguments = parse_args([
            ...
            "ryframe".into(),
            "--confirm-apply".into(),
            APPLY_CONFIRMATION.into(),
        ])
        .unwrap();
        assert_eq!(arguments.mode, Mode::Apply);
        assert_eq!(arguments.expected_database, "ryframe");
    }

    #[test]
    fn legacy_digests_are_strictly_validated() {
        assert_eq!(
            normalize_legacy_md5("9E107D9D372BB6826BD81D3542A419D6").unwrap(),
            "9e107d9d372bb6826bd81d3542a419d6"
        );
        for invalid in ["", "not-a-digest"] {
            assert!(normalize_legacy_md5(invalid).is_err());
        }
        assert!(normalize_legacy_md5(&"g".repeat(32)).is_err());
        assert!(validate_sha256(&"A".repeat(64)).is_err());
        assert!(validate_sha256(&"a".repeat(64)).is_ok());
    }

    #[test]
    fn md5_collision_still_produces_distinct_sha256_identities() {
        let first = calculate_object_digests(&collision_sample(
        ...
        let second = calculate_object_digests(&collision_sample(
            "d131dd02c5e6eec4693d9a0698aff95c\
        ...
        assert_eq!(first.legacy_md5, "79054025255fb1a26e4bc422aef54eb4");
        assert_eq!(first.legacy_md5, second.legacy_md5);
    ...
    #[test]
    fn backfill_validation_and_cas_decisions_are_fail_closed() {
        let digests = calculate_object_digests(b"legacy object");
        assert!(
            validate_backfill_object(
                7,
                i64::try_from(digests.byte_len).unwrap(),
                &digests.legacy_md5,
                &digests,
            )
            .is_ok()
        ...
        assert!(
            validate_backfill_object(
                7,
                i64::try_from(digests.byte_len + 1).unwrap(),
                &digests.legacy_md5,
                &digests,
            )
            .is_err()
        ...
        assert!(
            validate_backfill_object(7, digests.byte_len as i64, &"0".repeat(32), &digests)
                .is_err()
        );
        assert_eq!(
            classify_backfill_cas(Some(&digests.sha256), &digests.sha256),
            BackfillCasDecision::AlreadyApplied
        );
        assert_eq!(
            classify_backfill_cas(Some(&"f".repeat(64)), &digests.sha256),
            BackfillCasDecision::Conflict
        );
        assert_eq!(
            classify_backfill_cas(None, &digests.sha256),
            BackfillCasDecision::Conflict
        );
    }

    #[test]
    fn reservation_plan_is_fail_closed_and_reentrant() {
        let now = Utc::now();
        assert_eq!(
            plan_legacy_reservation(&row("ready", None), now).unwrap(),
            DrainPlan::NormalizeReady
        );
        assert_eq!(
            plan_legacy_reservation(
                &row("pending", Some(now - chrono::Duration::seconds(1))),
                now
            )
            .unwrap(),
            DrainPlan::MovePendingToCleanup
        ...
        assert_eq!(
            plan_legacy_reservation(
                &row("cleanup", Some(now - chrono::Duration::seconds(1))),
                now
            )
            .unwrap(),
            DrainPlan::DeleteCleanup
        );
        assert!(plan_legacy_reservation(&row("pending", None), now).is_err());
        assert!(plan_legacy_reservation(&row("unknown", Some(now)), now).is_err());
    }

    fn collision_sample(hex_value: &str) -> Vec<u8> {
        hex::decode(hex_value.split_whitespace().collect::<String>()).unwrap()
    }
}

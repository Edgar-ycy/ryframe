...
pub fn create_captcha_image(text: &str, width: u32, height: u32) -> AppResult<Vec<u8>> {
    ...
    let char_width = width / char_count;
    // 每个字符点阵在 scale=4 时占 20px 宽，居中放置
    let char_size = 20u32;
    let x_offset = if char_width > char_size {
        (char_width - char_size) / 2
    } else {
        0u32
    };
    let y_offset = (height.saturating_sub(char_size)) / 2;
    for (i, ch) in text.chars().enumerate() {
        let x = (i as u32) * char_width + x_offset;
        let y = y_offset;
        let text_color = Rgb([
...
fn draw_char(img: &mut ImageBuffer<Rgb<u8>, Vec<u8>>, ch: char, x: u32, y: u32, color: Rgb<u8>) {
    ...
    let pattern = match ch {
        ...
        '+' => vec![(2, 1), (0, 2), (1, 2), (2, 2), (3, 2), (4, 2), (2, 3)],
        '×' => vec![
            (0, 0),
            (4, 0),
            (1, 1),
        ...
        '?' => vec![(1, 0), (2, 0), (3, 0), (4, 1), (3, 2), (2, 3), (2, 4)],
        '=' => vec![
            (0, 1),
            (1, 1),
            (2, 1),

...
async fn list_accounts(
    ...
    let page = service(&state)?
        .list_accounts(&actor, query.into_page(state.settings.pagination)?)
        .await?;
...
async fn list_delegations(
    ...
    let page = service(&state)?
        .list_delegations(&actor, query.into_page(state.settings.pagination)?)
        .await?;
    Ok(Json(ApiPageResponse::page(
        page.records.into_iter().map(Into::into).collect(),
        page.total,
...
async fn list_access_audits(
    ...
    let page = service(&state)?
        .list_access_audits(&actor, query.into_page(state.settings.pagination)?)
        .await?;
    Ok(Json(ApiPageResponse::page(
        page.records.into_iter().map(Into::into).collect(),
        page.total,

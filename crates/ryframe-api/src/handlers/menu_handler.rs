...
async fn list(
    ...
    state.menu_service.find_filtered(&state.db, query.name.as_deref(), query.status.as_deref()).await.map(Json)
}
...
/// 菜单详情
async fn detail(State(state): State<AppState>, Path(id): Path<i64>) -> AppResult<Json<menu::Model>> {

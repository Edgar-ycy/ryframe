...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn DictPersistencePort> {
    Arc::new(LegacyDictPersistence { database })
}

struct LegacyDictPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyDictTransaction {
    transaction: sea_orm::DatabaseTransaction,
}

impl DictPersistencePort for LegacyDictPersistence {
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn DictTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyDictTransaction { transaction }) as Box<dyn DictTransaction>)
        })
    }
}

impl DictTransaction for LegacyDictTransaction {
...
impl ControlTransaction for LegacyDictTransaction {

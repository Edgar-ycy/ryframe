...
use ryframe_db::{ControlDatabaseCluster, ReadConsistency, entities::service_access_audit};
use ryframe_kernel::{PageResult, ValidatedPageQuery};
use sea_orm::{ColumnTrait, EntityTrait, PaginatorTrait, QueryFilter, QueryOrder, QuerySelect};

use crate::{PersistenceFuture, ServiceAccessAuditRecord, ServiceAccountAuditReadPort};

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn ServiceAccountAuditReadPort> {
    Arc::new(LegacyServiceAccountAuditPersistence { database })
}

struct LegacyServiceAccountAuditPersistence {
    database: ControlDatabaseCluster,
}

impl ServiceAccountAuditReadPort for LegacyServiceAccountAuditPersistence {
    fn list<'a>(
        &'a self,
        tenant_id: &'a str,

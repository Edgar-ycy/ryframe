...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn TenantConfigTransferPersistencePort> {
    Arc::new(LegacyTenantConfigTransferPersistence { database })
}

struct LegacyTenantConfigTransferPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyTenantConfigTransferTransaction {
    transaction: DatabaseTransaction,
}

impl TenantConfigTransferPersistencePort for LegacyTenantConfigTransferPersistence {
    ...
    fn load_resources<'a>(
        &'a self,
        tenant_id: &'a str,
    ) -> PersistenceFuture<'a, crate::system::TenantConfigPackageResources> {
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::resources::load_resources_on(
                self.database.write(),
                tenant_id,
            )
            .await
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn TenantConfigTransferTransaction>> {
        Box::pin(async move {
            let transaction = self
                ...
                .map_err(database_error)?;
            Ok(
                Box::new(LegacyTenantConfigTransferTransaction { transaction })
...
impl TenantConfigTransferTransaction for LegacyTenantConfigTransferTransaction {
    fn background_jobs(&self) -> &dyn crate::BackgroundJobTransaction {
        &self.transaction
    }

    fn product(&self) -> &dyn crate::ProductTransactionPort {
        &self.transaction
    }
    ...
    fn ensure_config_package_file_ready<'a>(
        ...
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::validation::ensure_config_package_file_ready_in_txn(
                &self.transaction,
                tenant_id,
                file_id,
                now,
    ...
    fn apply_resources<'a>(
        &'a self,
        tenant_id: &'a str,
        resources: &'a crate::system::TenantConfigPackageResources,
        plan_items: &'a [TenantConfigTransferItemRecord],
        now: chrono::DateTime<chrono::Utc>,
    ) -> PersistenceFuture<'a, ()> {
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::apply_resources::apply_resources_in_transaction(
                &self.transaction,
                tenant_id,
                resources,
                plan_items,
    ...
    fn ensure_rollback_references_safe<'a>(
        ...
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::rollback_resources::ensure_rollback_references_safe(
                &self.transaction,
                tenant_id,
                transfer_id,
            )
    ...
    fn restore_snapshot<'a>(
        &'a self,
        tenant_id: &'a str,
        snapshot: &'a crate::system::TenantConfigPackageResources,
        transfer_id: i64,
        target_catalog: &'a crate::system::TenantConfigTargetCatalog,
        now: chrono::DateTime<chrono::Utc>,
    ) -> PersistenceFuture<'a, ()> {
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::rollback_resources::restore_snapshot_in_transaction(
                &self.transaction,
                tenant_id,
                snapshot,
                transfer_id,
    ...
    fn ensure_requester_snapshot<'a>(
        ...
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::workflow_support::ensure_requester_snapshot_in_txn(
                &self.transaction,
                tenant_id,
                requester,
                fence,
    ...
    fn ensure_role_quota<'a>(
        ...
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::workflow_support::ensure_role_quota_for_plan_in_txn(
                &self.transaction,
                tenant_id,
                plan_items,
            )
    ...
    fn mark_plan_outcome<'a>(
        ...
        Box::pin(async move {
            crate::system::tenant_config_transfer_service::workflow_support::mark_plan_outcome(
                &self.transaction,
                tenant_id,
                transfer_id,
                outcome,
...
impl From<ryframe_db::TenantConfigurationFence> for TenantConfigurationFenceRecord {
    fn from(value: ryframe_db::TenantConfigurationFence) -> Self {
        Self {
...
mod tests {
    ...
    fn item_and_fence_mapping_preserve_boundaries() {
        ...
        let fence = TenantConfigurationFenceRecord::from(ryframe_db::TenantConfigurationFence {

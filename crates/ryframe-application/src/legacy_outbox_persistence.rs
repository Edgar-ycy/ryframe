...
use crate::{
    ...
    OutboxPersistencePort, PersistenceFuture, jobs::database_enqueue,
    legacy_execution_tenant_scope::database_scope,
};

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn OutboxPersistencePort> {
    Arc::new(LegacyOutboxPersistence {
        database,
        repository: OutboxEventRepository,
    })
}

struct LegacyOutboxPersistence {
    database: ControlDatabaseCluster,
    repository: OutboxEventRepository,
}

impl OutboxPersistencePort for LegacyOutboxPersistence {
    ...
    fn publish_audit<'a>(
        ...
        Box::pin(async move {
            ...
            let result = async {
                ...
                .await
                .inspect_err(|_| crate::record_audit_failure("oper_log_write"))?;
                self.repository
                    .mark_published_in_transaction(&transaction, event_id, worker_id, now)
                    .await
            }

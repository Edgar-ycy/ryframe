...
struct LegacyExportRequesterPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyExportRequesterTransaction {
    transaction: DatabaseTransaction,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn ExportRequesterPersistencePort> {
    Arc::new(LegacyExportRequesterPersistence { database })
}

impl ExportRequesterPersistencePort for LegacyExportRequesterPersistence {
    fn find<'a>(
        &'a self,
        tenant_id: &'a str,
        requester_id: i64,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn ExportRequesterTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyExportRequesterTransaction { transaction })
                as Box<dyn ExportRequesterTransaction>)
        })
    }
}

impl ExportRequesterTransaction for LegacyExportRequesterTransaction {
    fn database_now(&self) -> PersistenceFuture<'_, chrono::DateTime<chrono::Utc>> {
        Box::pin(async move {
            BackgroundJobRepository

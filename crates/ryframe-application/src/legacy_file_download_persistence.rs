...
use ryframe_db::{ControlDatabaseCluster, FileRepository, ReadConsistency};

use crate::{FileDownloadPersistencePort, FileDownloadRecord, PersistenceFuture};

struct LegacyFileDownloadPersistence {
    database: ControlDatabaseCluster,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn FileDownloadPersistencePort> {
    Arc::new(LegacyFileDownloadPersistence { database })
}

impl FileDownloadPersistencePort for LegacyFileDownloadPersistence {
...
fn map_record(file: ryframe_db::entities::sys_file::Model) -> FileDownloadRecord {
    FileDownloadRecord {
        bucket: file.bucket,

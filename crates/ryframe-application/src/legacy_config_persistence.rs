...
pub fn port(
    ...
    Arc::new(LegacyConfigPersistence {
        database,
        authorization_cache,
    })
}

struct LegacyConfigPersistence {
    database: ControlDatabaseCluster,
    authorization_cache: AuthorizationCache,
}

struct LegacyConfigTransaction {
    transaction: sea_orm::DatabaseTransaction,
    authorization_cache: AuthorizationCache,
}

impl ConfigPersistencePort for LegacyConfigPersistence {
    fn find_by_page<'a>(
        &'a self,
        tenant_id: &'a str,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn ConfigTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyConfigTransaction {
                transaction,
...
impl ConfigTransaction for LegacyConfigTransaction {
...
impl ControlTransaction for LegacyConfigTransaction {

...
struct LegacyFileUploadPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyFileUploadTransaction {
    transaction: DatabaseTransaction,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn FileUploadPersistencePort> {
    Arc::new(LegacyFileUploadPersistence { database })
}

impl FileUploadPersistencePort for LegacyFileUploadPersistence {
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn FileUploadTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyFileUploadTransaction { transaction })
...
impl FileUploadTransaction for LegacyFileUploadTransaction {
        ...
        Box::pin(async move {
            match mode {
                FileUploadCommitMode::CurrentRequest => {
                    crate::commit_current_audit(self.transaction).await
                }

...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn NoticePersistencePort> {
    Arc::new(LegacyNoticePersistence { database })
}

struct LegacyNoticePersistence {
    database: ControlDatabaseCluster,
}

struct LegacyNoticeTransaction {
    transaction: sea_orm::DatabaseTransaction,
}

impl NoticePersistencePort for LegacyNoticePersistence {
    fn find_by_id<'a>(
        &'a self,
        tenant_id: &'a str,
        id: i64,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn NoticeTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyNoticeTransaction { transaction }) as Box<dyn NoticeTransaction>)
        })
    }
}

impl NoticeTransaction for LegacyNoticeTransaction {
...
impl ControlTransaction for LegacyNoticeTransaction {

...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn BackgroundJobPersistencePort> {
    Arc::new(LegacyJobQueuePersistence {
        database,
        repository: BackgroundJobRepository,
    })
}

struct LegacyJobQueuePersistence {
    database: ControlDatabaseCluster,
    repository: BackgroundJobRepository,
}

impl BackgroundJobPersistencePort for LegacyJobQueuePersistence {
...
impl BackgroundJobTransaction for DatabaseTransaction {
    fn enqueue(&self, command: EnqueueJob) -> PersistenceFuture<'_, EnqueueJobResult> {
        Box::pin(async move {

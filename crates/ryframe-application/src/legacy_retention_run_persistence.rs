...
use ryframe_db::{ControlDatabaseCluster, DataRetentionRepository, entities::data_retention_run};
...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn RetentionRunPersistencePort> {
    Arc::new(LegacyRetentionRunPersistence {
        database,
        repository: DataRetentionRepository,
    })
}

struct LegacyRetentionRunPersistence {
    database: ControlDatabaseCluster,
    repository: DataRetentionRepository,
}

struct LegacyRetentionRunTransaction {
    transaction: DatabaseTransaction,
}

impl RetentionRunPersistencePort for LegacyRetentionRunPersistence {
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn RetentionRunTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyRetentionRunTransaction { transaction })
...
impl RetentionRunTransaction for LegacyRetentionRunTransaction {
    fn database_now(&self) -> PersistenceFuture<'_, chrono::DateTime<chrono::Utc>> {
        Box::pin(async move {

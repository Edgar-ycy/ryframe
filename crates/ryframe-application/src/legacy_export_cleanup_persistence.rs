...
struct LegacyExportCleanupPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyExportCleanupTransaction {
    transaction: DatabaseTransaction,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn ExportCleanupPersistencePort> {
    Arc::new(LegacyExportCleanupPersistence { database })
}

impl ExportCleanupPersistencePort for LegacyExportCleanupPersistence {
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn ExportCleanupTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyExportCleanupTransaction { transaction })
                as Box<dyn ExportCleanupTransaction>)
        })
    }
}

impl ExportCleanupTransaction for LegacyExportCleanupTransaction {

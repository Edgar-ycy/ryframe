...
impl AuthorizationCache {
    ...
    pub async fn increment_user_versions_in_transaction(
        ...
        let affected = transaction
            .increment_user_versions(tenant_id, &user_ids)
            .await?;
        if affected != user_ids.len() as u64 {
            return Err(AppError::NotFound("用户不存在".into()));
        }
        let versions = transaction.user_versions(tenant_id, &user_ids).await?;
    ...
    pub async fn increment_tenant_epoch_in_transaction(
        ...
        let authorization_epoch = transaction.increment_tenant_epoch(tenant_id).await?;
        if self.is_enabled() {
    ...
    pub async fn record_namespace_version_in_transaction(
        ...
        let namespace_version = transaction
            .increment_namespace_version(tenant_id, namespace)
            .await?;
        let now = transaction.database_now().await?;
...
) -> AppResult<AuthorizationMirrorEvent> {
    let trace_context = crate::trace_context::current_trace_context();
    Ok(AuthorizationMirrorEvent {
        tenant_id: tenant_id.to_owned(),

...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn UserQueryReadPort> {
    Arc::new(LegacyUserQueryPersistence { database })
}

struct LegacyUserQueryPersistence {
    database: ControlDatabaseCluster,
}

impl UserQueryReadPort for LegacyUserQueryPersistence {
    fn export_batch<'a>(
        &'a self,
        tenant_id: &'a str,
...
fn to_user(user: ryframe_db::entities::user::Model) -> UserQueryRecord {
    UserQueryRecord {
        id: user.id,

...
struct LegacyExportRequestPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyExportRequestTransaction {
    transaction: DatabaseTransaction,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn ExportRequestPersistencePort> {
    Arc::new(LegacyExportRequestPersistence { database })
}

impl ExportRequestPersistencePort for LegacyExportRequestPersistence {
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn ExportRequestTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyExportRequestTransaction { transaction })
                as Box<dyn ExportRequestTransaction>)
        })
    }
}

impl ExportRequestTransaction for LegacyExportRequestTransaction {
    ...
    fn find_active<'a>(
        ...
        Box::pin(async move {
            ExportJobRepository
                ...
                .await
                .map(|record| record.map(crate::legacy_export_requester_persistence::map_export))
        })
    }
    ...
    fn create_export(
        ...
        Box::pin(async move {
            ExportJobRepository
                ...
                .await
                .map(crate::legacy_export_requester_persistence::map_export)
        })
    }

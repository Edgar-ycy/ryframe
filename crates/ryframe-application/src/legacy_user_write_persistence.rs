...
pub fn port(
    ...
    Arc::new(LegacyUserWritePersistence {
        database,
        authorization_cache,
    })
}

struct LegacyUserWritePersistence {
    database: ControlDatabaseCluster,
    authorization_cache: crate::AuthorizationCache,
}

impl UserWritePersistencePort for LegacyUserWritePersistence {
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn UserWriteTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyUserWriteTransaction {
                transaction,
...
struct LegacyUserWriteTransaction {
    transaction: DatabaseTransaction,
    authorization_cache: crate::AuthorizationCache,
}

impl UserWriteTransaction for LegacyUserWriteTransaction {
...
fn to_user_record(user: user::Model) -> UserWriteRecord {
    UserWriteRecord {
        id: user.id,
        username: user.username,
        nickname: user.nickname,

...
struct LegacyPostPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyPostTransaction {
    transaction: sea_orm::DatabaseTransaction,
}

impl PostPersistencePort for LegacyPostPersistence {
    fn find_by_id<'a>(
        &'a self,
        tenant_id: &'a str,
        id: i64,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn PostTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyPostTransaction { transaction }) as Box<dyn PostTransaction>)
        })
    }
}

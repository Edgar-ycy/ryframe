...
struct LegacyExportArtifactPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyExportArtifactTransaction {
    transaction: DatabaseTransaction,
}

pub fn port(database: ControlDatabaseCluster) -> Arc<dyn ExportArtifactPersistencePort> {
    Arc::new(LegacyExportArtifactPersistence { database })
}

impl ExportArtifactPersistencePort for LegacyExportArtifactPersistence {
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn ExportArtifactTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyExportArtifactTransaction { transaction })
                as Box<dyn ExportArtifactTransaction>)
        })
    }
}

impl ExportArtifactTransaction for LegacyExportArtifactTransaction {

...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn MessagePersistencePort> {
    Arc::new(LegacyMessagePersistence { database })
}

struct LegacyMessagePersistence {
    database: ControlDatabaseCluster,
}

struct LegacyMessageTransaction {
    transaction: sea_orm::DatabaseTransaction,
}

impl MessagePersistencePort for LegacyMessagePersistence {
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn MessageTransaction>> {
        Box::pin(async move {
            let transaction = self
                ...
                .await
                .map_err(database_error)?;
            Ok(Box::new(LegacyMessageTransaction { transaction }) as Box<dyn MessageTransaction>)
        })
    }
}

impl MessageTransaction for LegacyMessageTransaction {
    fn publish(
        &self,
...
impl ControlTransaction for LegacyMessageTransaction {

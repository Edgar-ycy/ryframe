...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn OperLogPersistencePort> {
    Arc::new(LegacyOperLogPersistence { database })
}
...
struct LegacyOperLogPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyOperLogTransaction {
    transaction: DatabaseTransaction,
}

impl OperLogPersistencePort for LegacyOperLogPersistence {
    fn insert<'a>(
        &'a self,
        tenant_id: &'a str,
        record: OperLogRecord,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn OperLogTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyOperLogTransaction { transaction }) as Box<dyn OperLogTransaction>)
        })
    }
}

impl OperLogTransaction for LegacyOperLogTransaction {
...
impl ControlTransaction for LegacyOperLogTransaction {

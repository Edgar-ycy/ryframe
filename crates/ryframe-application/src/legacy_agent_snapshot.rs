use ryframe_db::{AgentRowScope as LegacyAgentRowScope, ServiceAuthorizationSnapshot};
...
pub(crate) fn authorization_snapshot(
    ...
    AgentAuthorizationSnapshot {
        user: snapshot.user.map(|user| AgentUserSnapshot {
            ...
            status: user.status,
            deleted: user.del_flag != ryframe_db::user::Model::DEL_FLAG_NORMAL,
        ...
        roles: snapshot
            ...
            .map(|role| AgentRoleSnapshot {
                ...
                status: role.status,
                deleted: role.del_flag != ryframe_db::role::Model::DEL_FLAG_NORMAL,
            })
            .collect(),
...
pub(crate) fn row_scope(scope: AgentRowScope) -> LegacyAgentRowScope {
    match scope {
        AgentRowScope::All => LegacyAgentRowScope::All,
        AgentRowScope::Departments(ids) => LegacyAgentRowScope::Departments(ids),
        AgentRowScope::DepartmentsAndUser {
            department_ids,
            user_id,
        } => LegacyAgentRowScope::DepartmentsAndUser {
            department_ids,
            user_id,
        },
        AgentRowScope::User(user_id) => LegacyAgentRowScope::User(user_id),
        AgentRowScope::Empty => LegacyAgentRowScope::Empty,
    }
}

...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn OverviewPersistencePort> {
    Arc::new(LegacyOverviewPersistence { database })
}

struct LegacyOverviewPersistence {
    database: ControlDatabaseCluster,
}

impl OverviewPersistencePort for LegacyOverviewPersistence {
...
fn map_counts(counts: Vec<ryframe_db::OverviewTrendCount>) -> Vec<OverviewTrendCount> {
    counts
        .into_iter()

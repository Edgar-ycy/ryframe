...
pub fn port(database: ControlDatabaseCluster) -> Arc<dyn LoginInfoPersistencePort> {
    Arc::new(LegacyLoginInfoPersistence { database })
}

struct LegacyLoginInfoPersistence {
    database: ControlDatabaseCluster,
}

struct LegacyLoginInfoTransaction {
    transaction: sea_orm::DatabaseTransaction,
}

impl LoginInfoPersistencePort for LegacyLoginInfoPersistence {
    fn insert<'a>(
        &'a self,
        tenant_id: &'a str,
    ...
    fn begin(&self) -> PersistenceFuture<'_, Box<dyn LoginInfoTransaction>> {
        Box::pin(async move {
            ...
            Ok(Box::new(LegacyLoginInfoTransaction { transaction })
                as Box<dyn LoginInfoTransaction>)
        })
    }
}

impl LoginInfoTransaction for LegacyLoginInfoTransaction {
...
impl ControlTransaction for LegacyLoginInfoTransaction {
